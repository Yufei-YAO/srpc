...
bool ByteArray::isLittleEndian() const {
    return m_endian == SRPC_LITTLE_ENDIAN;
}

void ByteArray::setIsLittleEndian(bool val) {
    if(val) {
        m_endian = SRPC_LITTLE_ENDIAN;
    } else {
        m_endian = SRPC_BIG_ENDIAN;
    }
}
...
void ByteArray::read(void* buf, size_t size) {
    ...
    Node* cur = m_root;
    int forward = m_position/m_baseSize;
    while(forward){
        forward--;
        cur = cur->next;
    }
...
void ByteArray::read(void* buf, size_t size, size_t position) const {
    ...
    size_t bpos = 0;
    Node* cur = m_root;
    int forward = position/m_baseSize;
    while(forward){
        forward--;
        cur = cur->next;
    }
